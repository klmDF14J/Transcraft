...
public class ColoredBlockUtil
{
	...
	public static void addLocalizationsForBlock(ItemBlock itemBlock, String suffix)
	{
		for(int i = 15; i >= 0; i--)
		{
			System.out.println(itemBlock.getUnlocalizedName(new ItemStack(itemBlock, 1, i)));
			LanguageRegistry.instance().addStringLocalization(itemBlock.getUnlocalizedName(new ItemStack(itemBlock, 1, i)) + ".name", getColorNameByMetadata(i) + " " + suffix);
			LanguageRegistry.addName(itemBlock, getColorNameByMetadata(i) + " " + suffix);
		}
	}
